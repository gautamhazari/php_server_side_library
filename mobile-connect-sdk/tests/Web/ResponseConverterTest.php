<?php

/**
 *                          SOFTWARE USE PERMISSION
 *
 *  By downloading and accessing this software and associated documentation
 *  files ("Software") you are granted the unrestricted right to deal in the
 *  Software, including, without limitation the right to use, copy, modify,
 *  publish, sublicense and grant such rights to third parties, subject to the
 *  following conditions:
 *
 *  The following copyright notice and this permission notice shall be included
 *  in all copies, modifications or substantial portions of this Software:
 *  Copyright © 2016 GSM Association.
 *
 *  THE SOFTWARE IS PROVIDED "AS IS," WITHOUT WARRANTY OF ANY KIND, INCLUDING
 *  BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY, FITNESS FOR A
 *  PARTICULAR PURPOSE AND NONINFRINGEMENT. IN NO EVENT SHALL THE AUTHORS OR
 *  COPYRIGHT HOLDERS BE LIABLE FOR ANY CLAIM, DAMAGES OR OTHER LIABILITY,
 *  WHETHER IN AN ACTION OF CONTRACT, TORT OR OTHERWISE, ARISING FROM, OUT OF OR
 *  IN CONNECTION WITH THE SOFTWARE OR THE USE OR OTHER DEALINGS IN THE
 *  SOFTWARE. YOU AGREE TO INDEMNIFY AND HOLD HARMLESS THE AUTHORS AND COPYRIGHT
 *  HOLDERS FROM AND AGAINST ANY SUCH LIABILITY.
 */
//require_once(dirname(__FILE__) . '/../bootstrap.php');

use MCSDK\Utils\RestResponse;
use MCSDK\MobileConnectStatus;
use MCSDK\Web\ResponseConverter;
use MCSDK\Discovery\DiscoveryResponse;
use MCSDK\Authentication\RequestTokenResponse;
use MCSDK\Authentication\RevokeTokenResponse;

class ResponseConverterTest extends PHPUnit_Framework_TestCase {

    const OPERATOR_SELECTION_URL = "http://discovery.sandbox2.mobileconnect.io/v2/discovery/users/operator-selection?session_id=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9._.2Lp0Xt9JXVZxNbnNq_RH-5KJPQ06qw6ttR4ZK3fwcQU";
    const AUTHORIZE_URL = "http://localhost:8080/authorize";

    private $_responses = array();

    public function __construct() {
        $this->_responses["operator-selection"] = new RestResponse(202, "{\"links\":[{\"rel\":\"operatorSelection\",\"href\":\"http://discovery.sandbox2.mobileconnect.io/v2/discovery/users/operator-selection?session_id=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9._.2Lp0Xt9JXVZxNbnNq_RH-5KJPQ06qw6ttR4ZK3fwcQU\"}]}");
        $this->_responses["authentication"] = new RestResponse(200, "{\"ttl\":1461169322705,\"subscriber_id\":\"\",\"response\":{\"serving_operator\":\"Example Operator A\",\"country\":\"US\",\"currency\":\"USD\",\"apis\":{\"operatorid\":{\"link\":[{\"href\":\"http://operator_a.sandbox2.mobileconnect.io/oidc/authorize\",\"rel\":\"authorization\"},{\"href\":\"http://operator_a.sandbox2.mobileconnect.io/oidc/accesstoken\",\"rel\":\"token\"},{\"href\":\"http://operator_a.sandbox2.mobileconnect.io/oidc/userinfo\",\"rel\":\"userinfo\"},{\"href\":\"openid profile email\",\"rel\":\"scope\"}]}},\"client_id\":\"66742a85-2282-4747-881d-ed5b7bd74d2d\",\"client_secret\":\"f15199f4-b658-4e58-8bb3-e40998873392\",\"subscriber_id\":\"\"}}");
        $this->_responses["error"] = new RestResponse(200, "{\"error\":\"Not_Found_Entity\",\"description\":\"Operator Not Found\"}");
        $this->_responses["token"] = new RestResponse(200, "{\"access_token\":\"966ad150-16c5-11e6-944f-43079d13e2f3\",\"token_type\":\"Bearer\",\"expires_in\":3600,\"id_token\":\"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..lwXhpEp2WUTi0brKBosM8Uygnrdq6FnLqkZ0Bm53gXA\"}");
        $this->_responses["invalid-code"] = new RestResponse(400, "{\"error\":\"invalid_grant\",\"error_description\":\"Authorization code doesn't exist or is invalid for the client\"}");
        $this->_responses["token_revoked"] = new RestResponse(200, "");
    }

    public function testResponseConverterShouldHandleOperatorSelectionStatus() {
        $status = MobileConnectStatus::OperatorSelection(self::OPERATOR_SELECTION_URL);
        $actual = ResponseConverter::Convert($status);

        $this->assertNotNull($actual);
        $this->assertEquals("success", $actual->getStatus());
        $this->assertEquals("operator_selection", $actual->getAction());
        $this->assertEquals(self::OPERATOR_SELECTION_URL, $actual->getUrl());
    }

    public function testResponseConverterShouldHandleStartAuthenticationStatus()
    {
        $session = "session";
        $response = new DiscoveryResponse($this->_responses["authentication"]);
        $status = MobileConnectStatus::StartAuthorization($response);
        $status->setSDKSession($session);

        $actual = ResponseConverter::Convert($status);

        $this->assertNotNull($actual);
        $this->assertEquals("success", $actual->getStatus());
        $this->assertEquals("start_authentication", $actual->getAction());
        $this->assertNotNull($actual->getSdkSession());
        $this->assertNotNull($actual->getSubscriberId());
    }

    public function testResponseConverterShouldHandleAuthenticationStatus()
    {
        $state = "qpwoeirurytalsksjdhdgf";
        $nonce = "qpwowieuryrttalksjsggdfj";
        $status = MobileConnectStatus::Authorization(self::AUTHORIZE_URL, $state, $nonce);

        $actual = ResponseConverter::Convert($status);

        $this->assertNotNull($actual);
        $this->assertEquals("success", $actual->getStatus());
        $this->assertEquals("authentication", $actual->getAction());
        $this->assertEquals(self::AUTHORIZE_URL, $actual->getUrl());
        $this->assertEquals($state, $actual->getState());
        $this->assertEquals($nonce, $actual->getNonce());
    }

    public function testResponseConverterShouldHandleCompleteStatus()
    {
        $response = new RequestTokenResponse($this->_responses["token"]);
        $status = MobileConnectStatus::Complete($response);

        $actual = ResponseConverter::Convert($status);

        $this->assertNotNull($actual);
        $this->assertEquals("success", $actual->getStatus());
        $this->assertEquals("complete", $actual->getAction());
        $this->assertEquals($status->getTokenResponse()->getResponseData(), $actual->getToken());
    }

    public function  testResponseConverterShouldHandleStartDiscoveryStatus()
    {
        $status = MobileConnectStatus::StartDiscovery();

        $actual = ResponseConverter::Convert($status);

        $this->assertNotNull($actual);
        $this->assertEquals("success", $actual->getStatus());
        $this->assertEquals("discovery", $actual->getAction());
    }

    public function testResponseConverterShouldHandleErrorStatus()
    {
        $error = "this is the error";
        $description = "this is the description";
        $status = MobileConnectStatus::Error($error, $description, null);

        $actual = ResponseConverter::Convert($status);

        $this->assertNotNull($actual);
        $this->assertEquals("failure", $actual->getStatus());
        $this->assertEquals("error", $actual->getAction());
        $this->assertEquals($error, $actual->getError());
        $this->assertEquals($description, $actual->getDescription());
    }

    public function testResponseConverterShouldHandleTokenRevokedStatus()
    {
        $response = new RevokeTokenResponse($this->_responses["token_revoked"]);
        $status = MobileConnectStatus::TokenRevoked($response);

        $actual = ResponseConverter::Convert($status);

        $this->assertNotNull($actual);
        $this->assertEquals("success", $actual->getStatus());
        $this->assertEquals("token_revoked", $actual->getAction());
    }
}
